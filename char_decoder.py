...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        h_t, c_t = self.charDecoder(self.decoderCharEmb(input), dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        scores, c_t = self.forward(char_sequence[:-1, :], dec_hidden)
        scores = scores.flatten(0, 1)
        char_sequence_f = char_sequence[1:, :].flatten()
        loss_func = nn.CrossEntropyLoss(reduction='sum', ignore_index=0)
        return loss_func(scores, char_sequence_f)
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        next_c = torch.tensor([self.target_vocab.start_of_word] * batch_size, device=device).unsqueeze(0)
        c_t = initialStates
        output = []
        for t in range(max_length - 1):
            scores, c_t = self.forward(next_c, c_t)
        ...
        for word in output:
            ...
            for char in word:
                ...
                if char != self.target_vocab.end_of_word:
                    word_trunc += self.target_vocab.id2char[char]
                else:
                    break
            decodedWords.append(word_trunc)
        return decodedWords
